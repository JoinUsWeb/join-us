...
class Search_activity extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $select['first_label_id'] = isset($select['first_label_id']) ? $select['first_label_id'] : 0;
        ...
        if ($select['second_label_id'] != 0)
            $select['first_label_id'] = $this->First_label_model->get_first_label_by_second_id($select['second_label_id'])['id'];

        $activity = $this->Search_activity_model->search_activity($select['first_label_id'],$select['second_label_id'], /*$select['city']*/
            0, $select['time'], $select['order']);
